...
<div class="container-fluid" >
	...
	<div class="table-responsive">
		...
		<table class="table table-hover tabla-equipos" id="SilaisTableEquipos" style="width:1850px;">
			<thead>
				<tr>
					<th>Acciones</th>
					<th>Unidad</th>
...
$(document).ready(function() {
    ...
    $('#SilaisTableEquipos tfoot th').each( function () {

        if( $('#SilaisTableEquipos thead th').eq( $(this).index() ).text()!="Acciones"){
            var title = $('#SilaisTableEquipos thead th').eq( $(this).index() ).text();
            $(this).html( '<input type="text" class="form-control" placeholder="Buscar por '+ title +'" />' );  
        }
    } );

    // DataTable
    var table = $('#SilaisTableEquipos').DataTable({
        "language":{
    ...
    $('#SilaisTableEquipos_filter').css("display","none");
    // Aplicar Busqueda
